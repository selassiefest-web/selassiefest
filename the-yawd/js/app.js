// Fill these in before sending this link to an investor.
const CONFIG = {
...
function voiceLinesFor(frame) {
  ...
  if (frame.personalize === 'close') {
    return frame.voice.map((line) => {
      ...
      return name ? interpolate(line, name) : line.replace('{{name}}, ', '');
    });
  }
  return frame.voice;
}
  ...
  FRAMES.forEach((frame, i) => {
    ...
    if (frame.cta) {
        ...
        <div class="frame-content-inner">
          ...
          <div class="cta-card">
            <div class="cta-buttons">
              <a class="cta-btn primary" href="mailto:${CONFIG.contactEmail}">Email MarlonTRC</a>
              ${CONFIG.scheduleUrl ? `<a class="cta-btn secondary" href="${CONFIG.scheduleUrl}" target="_blank" rel="noopener">Schedule a call</a>` : ''}
            </div>
            ${CONFIG.contactEmail.includes('example.com') ? '<div class="cta-fill-note">Set CONFIG.contactEmail / scheduleUrl in js/app.js before sending this to an investor.</div>' : ''}
          </div>
        </div>
      `;
    } else {
      ...
      content.innerHTML = `
        <div class="frame-content-inner">
          <span class="batch-label">${BATCH_LABELS[frame.batch] || ''}</span>
          ${frame.proof ? '<span class="proof-badge">Live today, not a projection</span>' : ''}
...
function speakFrame(frame) {
  ...
  const isMoneyFrame = frame.batch === 'money';
  const rate = isMoneyFrame ? 0.85 : 0.92;
  const gapMs = isMoneyFrame ? 650 : 350;
