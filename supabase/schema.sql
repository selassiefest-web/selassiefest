...
values
  ('14-parish-restaurant-and-rhum-bar', '14 Parish Restaurant & Rhum Bar', '1644 E 53rd St, Chicago, IL 60615', '(312) 291-8379', 'info@14parish.com', 'https://www.14parish.com', null, null, null, 'Donation of 2 Dinner Certificates', '$100', 'Dinner for Two Certificates', 'Not Contacted', 'High', 'Upscale Caribbean restaurant; strong auction appeal'),
  ...
  ('jamaica-jerk-king', 'Jamaica Jerk King', '206 E 35th St, Chicago, IL 60616', '(312) 754-0480', null, null, null, null, null, 'Dinner for Two Certificate', '$50', 'Dinner for Two Certificate', 'Not Contacted', 'Medium', 'Bronzeville location; longtime South Side staple; solid auction appeal; walk-in approach may work well'),
...
-- ─────────────────────────────────────────────────────────────────────────
-- Plates for Purpose — personalized partner badges
-- ─────────────────────────────────────────────────────────────────────────
-- badge_path: a "Proud Plates for Purpose Partner" social-media-ready badge
-- image, generated by compositing that restaurant's own logo onto a shared
-- template (see supabase/generate-partner-badges.py, run manually whenever
-- new logos are added). Attached automatically to the "yes" confirmation
-- email (notify-submission's plates_for_purpose_responses config) so a
-- restaurant has something ready to post the moment they confirm.
alter table plates_for_purpose_restaurants add column if not exists badge_path text;

create or replace view plates_for_purpose_restaurants_public as
select slug, business_name, address, donation_ask, target_ask_value, suggested_donation, logo_path, offer_choices, offer_note, badge_path
from plates_for_purpose_restaurants;
...
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values (
  'plates-for-purpose-badges', 'plates-for-purpose-badges', true, 5242880,
