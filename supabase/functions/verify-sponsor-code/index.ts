// Called from the 13 sponsor inquiry pages after the user types in the
// code emailed to them by request-sponsor-verification. On success, marks
// the email as verified -- which is what sponsor_email_is_verified() (used
// by sponsor_inquiries' INSERT policy) checks before allowing the real
// submit. Mirrors verify-comp-code.
...
// See request-sponsor-verification for why these are required on every response.
const corsHeaders = {
...
Deno.serve(async (req) => {
  ...
  if (codeHash !== row.code_hash) {
    await supabase.from("sponsor_verifications").update({ attempts: row.attempts + 1 }).eq("email", normalizedEmail);
    return new Response(JSON.stringify({ valid: false, error: "That code doesn't match" }), { status: 200, headers: jsonHeaders });
  }

  await supabase.from("sponsor_verifications").update({ verified: true }).eq("email", normalizedEmail);
