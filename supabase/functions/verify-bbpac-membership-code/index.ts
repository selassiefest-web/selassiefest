// Called from /bbpac/get-involved/membership.html after the user types in
// the code emailed to them by request-bbpac-membership-verification. On
// success, marks the email as verified -- which is what
// bbpac_membership_email_is_verified() (used by bbpac_membership_signups'
// INSERT policy) checks before allowing the real submit. Mirrors
...
Deno.serve(async (req) => {
  ...
  if (codeHash !== row.code_hash) {
    await supabase.from("bbpac_membership_verifications").update({ attempts: row.attempts + 1 }).eq("email", normalizedEmail);
    return new Response(JSON.stringify({ valid: false, error: "That code doesn't match" }), { status: 200, headers: jsonHeaders });
  }

  await supabase.from("bbpac_membership_verifications").update({ verified: true }).eq("email", normalizedEmail);
