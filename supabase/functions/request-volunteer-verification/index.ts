// Called from /contact/volunteers.html before it lets someone submit the
// volunteer application. Sends a 6-digit code by email (never a clickable
// link -- those get silently consumed by mail scanners before the human
// clicks them). Uses the service role internally since the caller is
// anonymous and volunteer_verifications has no public policies at all.
// Mirrors request-comp-verification.
...
const RATE_LIMIT_PURPOSE = "volunteer";
...
Deno.serve(async (req) => {
  ...
  const emailRes = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: FROM,
      to: [email.trim()],
      subject: "Your SelassieFest volunteer verification code",
      html: `
        <p>Here's your verification code for the SelassieFest volunteer application:</p>
