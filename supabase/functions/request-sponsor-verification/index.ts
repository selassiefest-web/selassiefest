// Called from the 13 sponsor inquiry pages before letting someone submit.
// Sends a 6-digit code by email (never a clickable link -- those get
// silently consumed by mail scanners before the human clicks them). Uses
// the service role internally since the caller is anonymous and
// sponsor_verifications has no public policies at all. Mirrors
// request-comp-verification.
...
Deno.serve(async (req) => {
  ...
  const emailRes = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: FROM,
      to: [email.trim()],
      subject: "Your SelassieFest sponsor inquiry verification code",
      html: `
        <p>Here's your verification code for your SelassieFest sponsor inquiry:</p>
