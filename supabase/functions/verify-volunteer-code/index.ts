// Called from /contact/volunteers.html after the user types in the code
// emailed to them by request-volunteer-verification. On success, marks the
// email as verified -- which is what volunteer_email_is_verified() (used by
// volunteer_signups' INSERT policy) checks before allowing the real submit.
// Mirrors verify-comp-code.
...
// See request-volunteer-verification for why these are required on every response.
const corsHeaders = {
...
Deno.serve(async (req) => {
  ...
  if (codeHash !== row.code_hash) {
    await supabase.from("volunteer_verifications").update({ attempts: row.attempts + 1 }).eq("email", normalizedEmail);
    return new Response(JSON.stringify({ valid: false, error: "That code doesn't match" }), { status: 200, headers: jsonHeaders });
  }

  await supabase.from("volunteer_verifications").update({ verified: true }).eq("email", normalizedEmail);
