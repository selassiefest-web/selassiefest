...
const RATE_LIMIT_PURPOSE = "bbpac_directory";
...
// blocks the whole request before it reaches this code at all. Origin is
// locked to selassiefest.com (not "*") so a script on some other site can't
// drive a visitor's browser into requesting codes on their behalf.
const corsHeaders = {
...
// bbpac_membership_signups.email is stored exactly as the member typed it
// (membership.html only trims, never lowercases), so this lookup needs
// case-insensitive matching -- hence .ilike() rather than .eq(). But
// .ilike() treats its argument as a LIKE pattern, and this argument is raw
// caller input: an unescaped "%" or "_" turns the check into a wildcard
// that matches ANY row, letting a non-member get a valid code (and, via
// verify-bbpac-directory-code, the entire member list) just by typing "%"
// as their "email". Escaping LIKE metacharacters keeps the match exact
// (case-insensitively) instead of a pattern.
...
Deno.serve(async (req) => {
  ...
  // From here on, every path -- member or not, in cooldown or not -- returns
  // the exact same { ok: true } shape. This endpoint's whole job is "prove
  // you own an email that's already a member before showing the directory,"
  // so a response that varies by membership status (a distinct "not found"
  // error, a distinct "please wait" cooldown message visible only once a
  // code has actually been sent) lets anyone learn who's on the Friends of
  // Bongo Beach list just by trying emails here -- no code, no ownership
  // proof required. Genuine infra failures (Resend/DB errors) still surface
  // below, since those only happen inside the member+send-a-code branch;
  // that's an accepted, low-probability residual signal, not something
  // fixable without hiding real outages from real members too.
  if (member) {
    ...
    const secondsSinceLast = existing ? (Date.now() - new Date(existing.created_at).getTime()) / 1000 : Infinity;
    ...
    // else: a still-live code was already sent recently -- don't send
    // another, but still fall through to the identical response below.
  }
